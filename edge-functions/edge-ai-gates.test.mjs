...
function indexOfAfter(source, needle, fromIndex) {
...
assertBefore(
  ...
  senseiChat.indexOf("const anthropicPayload"),
);
assertNotContains(senseiChat, 'sb.from("ai_rate_log").insert({ user_id: userId }).then');
...
assert(whatsapp.includes("if (!accessRes.data?.ai_enabled)"));
