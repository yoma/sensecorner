...
async function mayUseSenseiDuringOnboarding(userId: string): Promise<boolean> {
  ...
  if (completedRes.error) {
    console.warn("whatsapp onboarding gate completed check", completedRes.error.message);
    return false;
  }
  ...
  const startedRes = await sb
    ...
    .maybeSingle();
  if (startedRes.error) {
    console.warn("whatsapp onboarding gate started check", startedRes.error.message);
    return false;
  }
...
async function resolveAiAccessForUser(userId: string): Promise<AiAccessDecision> {
  ...
  if (!aiEnabled) aiEnabled = await mayUseSenseiDuringOnboarding(userId);
  if (!aiEnabled) {
    return {
      allowed: false,
